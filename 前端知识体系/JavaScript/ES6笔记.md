...
* 注意事项
	...
	* 4.不能使用`yield`命令，不能用作`Generator`函数
### set、map
...
```javascript
let [a, b, c] = [1, 2, 3]
let [a, [b, c]] = [1, [2, 3]] // 嵌套数组解构
let [a, ...g] = [1, 2, 3] // 数组拆分 a = 1, g = [2, 3]
let [a, , c] = [1, 2, 3] // 不连续解构 a = 1, c = 3
...
* `forEach`更多的用来遍历数组
* `for in `一般常用来遍历对象或`json`，循环出的是`key`
* `for of`数组对象都可以遍历，遍历对象需要通过和`Object.keys()`，循环出的是`value`
...
用给定名称作为唯一标识，确保唯一，即使采用相同的名称，也会产生不同的值